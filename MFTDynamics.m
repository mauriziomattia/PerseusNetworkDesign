function [t, Nu, NuN] = MFTDynamics(Nu0, t0, tMax, Dt, Net, DNuExt)
%
%  [t, Nu, NuN] = MFTDynamics(Nu0, t0, tMax, Dt, Net[, DNuExt])
%
% Version 1.0, 4 Dec. 2006
% Copyright (c) Maurizio Mattia, maurizio.mattia@iss.it

ONLINE_PLOT = 0;
NO_FINITE_SIZE_NOISE = 0;
...
Nu(:,1) = Nu0;
if NO_FINITE_SIZE_NOISE
   NuN(:,1) = Nu0;
else
   NuN(:,1) = poissrnd(Net.SNParam.N .* Nu0 * Dt) ./ (Net.SNParam.N * Dt);
end
%NuN(:,1) = binornd(Net.SNParam.N, Nu0*Dt) ./ (Net.SNParam.N*Dt);
%WhiteNoise = normrnd(0, 1, Net.P, floor((tMax-t0)/Dt)+1);
%NuN(:,1) = Nu(:,1) + WhiteNoise(:,1).*sqrt(Nu(:,1)./(Net.SNParam.N*Dt));
NuNMean(:,1) = Nu0;
...
for n = 2:length(t)
   ...
   if NO_FINITE_SIZE_NOISE
      ...
      NuN(:,n) = poissrnd(Net.SNParam.N .* Nu(:,n) * Dt) ./ (Net.SNParam.N * Dt);
   end
   %   NuN(:,n) = binornd(Net.SNParam.N, Nu(:,n)*Dt) ./ (Net.SNParam.N*Dt);
   %   NuN(:,n) = Nu(:,n) + WhiteNoise(:,n) .* sqrt(Nu(:,n) ./ (Net.SNParam.N * Dt));
